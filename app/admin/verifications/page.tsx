...
async function getPendingInstructors() {
  ...
  const instructors = await sql`
    SELECT 
      ...
      i.created_at,
      (SELECT COUNT(*) FROM instructor_documents WHERE instructor_id = i.id) as doc_count,
      (SELECT COUNT(*) FROM instructor_documents WHERE instructor_id = i.id AND status = 'pending') as pending_docs
    FROM instructors i
    WHERE i.verification_status IN ('under_review', 'pending')
