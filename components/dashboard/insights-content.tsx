...
export function InsightsContent() {
  ...
  const handleSaveAssessment = async () => {
    if (!selectedStudent || !generatedSummary) return

    setIsSaving(true)
    try {
      const response = await fetch("/api/lessons/save-assessment", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          studentId: selectedStudent,
          skillRatings: skillRatings.filter((s) => s.rating > 0),
      ...
      setSkillRatings(AVAILABLE_SKILLS.map((skill) => ({ skill, rating: 0 })))
      setGeneratedSummary(null)
      setHasRated(false)
      alert("Assessment saved successfully! Student will see this in their profile.")
    } catch (error) {
          ...
          {generatedSummary && hasRated && (
            <div className="space-y-4">
              ...
              <div className="flex gap-3">
                ...
                <Button onClick={handleSaveAssessment} disabled={isSaving} className="flex-1">
